...
@teams_sender("https://livelancsac.webhook.office.com/webhookb2/3d6a96cd-dfcc-4879-a4b4-fbf819b2b0aa@9c9bcd11-977a-4e9c-a9a0-bc734090164a/IncomingWebhook/fc46931452294793b7c5f4fa55c75b07/e07e4eb0-8af1-4f92-8974-01147fb1408a")
def inference_rippler (test_results:np.array,N:int,h:np.array,gamma:float,T:int,seasonal_matrix_G:np.array,seasonal_matrix_H:np.array,age:np.array,sex:np.array,sens:float,spec:float,theta_start:np.array,X_start:np.array,epsilon_0:float,epsilon_1:float,L:int,mu:np.array,prior_X_0:float,K:int,K_latent:int,K_chunk:int,zarr_names_start:str,seed:int) :
    ...
    # creating the inital mass matrix
    mass = np.identity(4)
    mass_inverse = mass
    epsilon = epsilon_0
    ...
    for k_chunk in range(K_chunk):
        ...
        for k in range(k_start,k_end):
            ...
            p = random.multivariate_normal(np.zeros(4),mass)
            ...
            for t in range(L):
                ...
                theta_moving = theta_moving + epsilon*np.matmul(mass_inverse,p_half)
        ...
        if k_chunk==0:
            ...
            covariance = (1/(chunk_size-1))*np.matmul(np.transpose(K_tilda),K_tilda)
            # mass = np.linalg.inv(covariance)
            mass = np.identity(4)
            mass[0,0] = 1/covariance[0,0]
            mass[1,1] = 1/covariance[1,1]
            mass[2,2] = 1/covariance[2,2]
            mass[3,3] = 1/covariance[3,3]
