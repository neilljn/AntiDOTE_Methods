...
from function_scripts.rippler.log_dis_theta_maker import log_dis_theta_maker
from function_scripts.rippler.grad_log_dis_theta_maker import grad_log_dis_theta_maker
...
# function for 1 leapfrog step
def leapfrog(theta:np.array, p:np.array, epsilon:float, grad_log_dis_theta):
    p_half = p + epsilon*0.5*grad_log_dis_theta(theta)
    theta_prime = theta+epsilon*p_half
    p_prime = p_half + epsilon*0.5*grad_log_dis_theta(theta_prime)
    return theta_prime, p_prime

# function for building a tree
def build_tree(theta:np.array, p:np.array, log_u:float, v:int, j:int, epsilon:float, delta_max:float, log_dis_theta, grad_log_dis_theta):

    if j == 0 :
        
        # base case - move one step in the direction of v
        theta_prime, p_prime = leapfrog(theta, p, v*epsilon, grad_log_dis_theta)
        n_prime = (log_u <= (log_dis_theta(theta_prime) - 0.5*np.sum(p_prime**2)))
        s_prime = (log_u < (delta_max + log_dis_theta(theta_prime) - 0.5*np.sum(p_prime**2)))
        return theta_prime, p_prime, theta_prime, p_prime, theta_prime, n_prime, s_prime
    
    else:
        
        # recursively build the tree
        theta_minus, p_minus, theta_plus, p_plus, theta_prime, n_prime, s_prime = build_tree(theta, p, log_u, v, j-1, epsilon, delta_max, log_dis_theta, grad_log_dis_theta)
        if s_prime == 1 :
            if v == -1 :
                # building backwards
                theta_minus, p_minus, blank_1, blank_2, theta_double_prime, n_double_prime, s_double_prime = build_tree(theta_minus, p_minus, log_u, v, j-1, epsilon, delta_max, log_dis_theta, grad_log_dis_theta)
            else:
                # building forwards
                blank_1, blank_2, theta_plus, p_plus, theta_double_prime, n_double_prime, s_double_prime = build_tree(theta_plus, p_plus, log_u, v, j-1, epsilon, delta_max, log_dis_theta, grad_log_dis_theta)
            u_ast = random.uniform()
            if u_ast < (n_double_prime/(n_prime+n_double_prime)) :
                theta_prime = theta_double_prime
            s_prime = s_double_prime * (np.dot(theta_plus-theta_minus,p_minus) >= 0) * (np.dot(theta_plus-theta_minus,p_plus) >= 0)
            n_prime = n_prime + n_double_prime
        return theta_minus, p_minus, theta_plus, p_plus, theta_prime, n_prime, s_prime
    
# run 1 iteration of efficient NUTS
def efficient_NUTS(theta:np.array, epsilon:float, delta_max:float, d:int, log_dis_theta, grad_log_dis_theta):

    # initialisation
    p_0 = random.multivariate_normal(np.zeros(d),np.identity(d))
    #u = random.uniform(0,np.exp(log_dis_theta_jit(theta) - 0.5*np.sum(p_0**2)))
    log_u = log_dis_theta(theta) - 0.5*np.sum(p_0**2) - random.exponential(1)
    theta_minus = theta
    theta_plus = theta
    p_minus = p_0
    p_plus = p_0
    ...
    while s == 1 :
        ...
        v_j = random.choice([-1,1])

        if v_j == -1 :
            # building backwards
            theta_minus, p_minus, blank_1, blank_2, theta_prime, n_prime, s_prime = build_tree(theta_minus, p_minus, log_u, v_j, j, epsilon, delta_max, log_dis_theta, grad_log_dis_theta)
        else:
            # building forwards
            blank_1, blank_2, theta_plus, p_plus, theta_prime, n_prime, s_prime = build_tree(theta_plus, p_plus, log_u, v_j, j, epsilon, delta_max, log_dis_theta, grad_log_dis_theta)

        if s_prime == 1 :
            u_ast = random.uniform()
            if u_ast < (n_prime/n):
                theta_out = theta_prime
        
        n = n + n_prime
        s = s_prime * (np.dot(theta_plus-theta_minus,p_minus) >= 0) * (np.dot(theta_plus-theta_minus,p_plus) >= 0)
        j = j + 1

    # returning the final theta
    return theta_out

# MCMC algorithm for inference using u instead of C
@teams_sender("https://livelancsac.webhook.office.com/webhookb2/3d6a96cd-dfcc-4879-a4b4-fbf819b2b0aa@9c9bcd11-977a-4e9c-a9a0-bc734090164a/IncomingWebhook/fc46931452294793b7c5f4fa55c75b07/e07e4eb0-8af1-4f92-8974-01147fb1408a")
def inference_rippler (test_results:np.array,N:int,h:np.array,gamma:float,T:int,seasonal_matrix_G:np.array,seasonal_matrix_H:np.array,age:np.array,sex:np.array,sens:float,spec:float,theta_start:np.array,X_start:np.array,epsilon:float,delta_max:float,mu:np.array,prior_X_0:float,K:int,K_latent:int,K_chunk:int,zarr_names_start:str,seed:int) :
    ...
    # epsilon - jump size in the HMC dynamics
    # delta_max - accuracy bound
    ...
    for k_chunk in range(K_chunk):
        ...
        for k in range(k_start,k_end):
            ...
            # creating the likelihood and gradient functions
            log_dis_theta = log_dis_theta_maker(X,seasonal_matrix_G,seasonal_matrix_H,age,sex,T,N,h,mu)
            grad_log_dis_theta = grad_log_dis_theta_maker(X,seasonal_matrix_G,seasonal_matrix_H,age,sex,T,N,h,mu)
            log_dis_theta_jit = jit(log_dis_theta)
            grad_log_dis_theta_jit = jit(grad_log_dis_theta)

            # runnning NUTS
            theta = efficient_NUTS(theta, epsilon, delta_max, 4, log_dis_theta_jit, grad_log_dis_theta_jit)
            ll_curr = 0
            ...
            X_prop = X_from_U_jit(U,np.array([X_0_prop[0]]),age,sex,theta,gamma,N,T,h)
            ...
            for k_latent in range(K_latent):
                ...
                log_q_move = U_prop_bounds['log_q_move']
                
                # calculating the log likelihood based on the proposed u
                X_prop = X_from_U_jit(U_prop,np.array([X[0]]),age,sex,theta,gamma,N,T,h)
                like_prop = log_dis_U_jit(X_prop,test_results,sens,spec)
            ...
            # like_initial = 0
            # like = 0
            # acc_latent_times_total = np.repeat(1,T)
