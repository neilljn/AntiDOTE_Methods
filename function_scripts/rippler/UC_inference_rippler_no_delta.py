...
@teams_sender("https://livelancsac.webhook.office.com/webhookb2/3d6a96cd-dfcc-4879-a4b4-fbf819b2b0aa@9c9bcd11-977a-4e9c-a9a0-bc734090164a/IncomingWebhook/fc46931452294793b7c5f4fa55c75b07/e07e4eb0-8af1-4f92-8974-01147fb1408a")
def inference_rippler (test_results:np.array,N:int,h:np.array,gamma:float,delta:np.array,T:int,seasonal_matrix_G:np.array,seasonal_matrix_H:np.array,age:np.array,sex:np.array,sens:float,spec:float,theta_start:np.array,X_start:np.array,covariance_start:np.array,scaling:float,mu:np.array,prior_X_0:float,K:int,K_latent:int,K_chunk:int,zarr_names_start:str,seed:int) :
    ...
    theta_store = zarr.open(zarr_names_start+'/theta.zarr', mode='w', shape=(K, 2), chunks=(chunk_size, 2))
    ...
    covariance_store = zarr.open(zarr_names_start+'/covariance.zarr', mode='w', shape=(K_chunk, 2, 2), chunks=(chunk_size, 2, 2))
    ...
    for k_chunk in range(K_chunk):
        ...
        theta_store_temp = np.zeros((chunk_size,2))
        ...
        for k in range(k_start,k_end):
            ...
            theta_prop = random.multivariate_normal(theta[0:2],covariance)
            theta_prop = np.hstack((theta_prop,delta))
            ...
            k_temp = k % chunk_size
            theta_store_temp[k_temp] = theta[0:2]
        ...
        K_expected = np.tile([beta_G_mean,beta_H_mean],(chunk_size,1))
