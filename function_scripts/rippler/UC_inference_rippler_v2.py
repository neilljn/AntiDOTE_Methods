...
@teams_sender("https://livelancsac.webhook.office.com/webhookb2/3d6a96cd-dfcc-4879-a4b4-fbf819b2b0aa@9c9bcd11-977a-4e9c-a9a0-bc734090164a/IncomingWebhook/fc46931452294793b7c5f4fa55c75b07/e07e4eb0-8af1-4f92-8974-01147fb1408a")
def inference_rippler_v2 (test_results:np.array,N:int,h:np.array,gamma:float,T:int,seasonal_matrix_G:np.array,seasonal_matrix_H:np.array,age:np.array,sex:np.array,sens:float,spec:float,theta_start:np.array,X_start:np.array,covariance_start:np.array,mu:np.array,prior_X_0:float,K:int,K_latent:int,K_chunk:int,zarr_names_start:str,seed:int) :
    ...
    for k_chunk in range(K_chunk):
        ...
        for k in range(k_start,k_end):
            ...
            like = log_dis_U_jit(X,test_results,sens,spec) + 2*np.sum(np.log(U_bounds['upper']-U_bounds['lower']))
            ...
            U_bounds_prop = U_bounds_from_X_jit(X_prop,seasonal_matrix_G,seasonal_matrix_H,age,sex,theta,gamma,T,N,h)
            like_prop = log_dis_U_jit(X_prop,test_results,sens,spec) + 2*np.sum(np.log(U_bounds_prop['upper']-U_bounds_prop['lower']))
