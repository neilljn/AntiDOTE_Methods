...
@teams_sender("https://livelancsac.webhook.office.com/webhookb2/3d6a96cd-dfcc-4879-a4b4-fbf819b2b0aa@9c9bcd11-977a-4e9c-a9a0-bc734090164a/IncomingWebhook/fc46931452294793b7c5f4fa55c75b07/e07e4eb0-8af1-4f92-8974-01147fb1408a")
def inference_rippler (test_results:np.array,N:int,h:np.array,gamma:float,T:int,seasonal_matrix_G:np.array,seasonal_matrix_H:np.array,age:np.array,sex:np.array,sens:float,spec:float,theta_start:np.array,X_start:np.array,covariance_start:np.array,a:float,nu_0:float,f,lambda_min:float,mu:np.array,prior_X_0:float,K:int,K_latent:int,K_chunk:int,zarr_names_start:str,seed:int) :
    ...
    n_start = 5/(a*(1-a))
    A = -np.sqrt(2*np.log(np.sqrt(2/np.pi)/a))
    delta = (1-1/d)*(np.sqrt(2*np.pi)*np.exp(A*A/2)/(2*A)) + 1/(d*a*(1-a))
    theta_mean = theta
    ...
    for k_chunk in range(K_chunk):
        ...
        for k in range(k_start,k_end):
            ...
            theta_prop = random.multivariate_normal(theta,c*(lambda_current**2)*covariance)
            ...
            # updating the scale
            lambda_current = np.max([lambda_min, lambda_current*np.exp((np.exp(log_alpha)-a)*delta/(n_start+n))])

            # potentiallly resetting
            if np.absolute(np.log(lambda_current)-np.log(lambda_start))>np.log(3) :
