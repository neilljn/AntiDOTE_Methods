...
from function_scripts.rippler.grad_potential import grad_potential_jit
...
@teams_sender("https://livelancsac.webhook.office.com/webhookb2/3d6a96cd-dfcc-4879-a4b4-fbf819b2b0aa@9c9bcd11-977a-4e9c-a9a0-bc734090164a/IncomingWebhook/fc46931452294793b7c5f4fa55c75b07/e07e4eb0-8af1-4f92-8974-01147fb1408a")
def inference_rippler (test_results:np.array,N:int,h:np.array,gamma:float,T:int,seasonal_matrix_G:np.array,seasonal_matrix_H:np.array,age:np.array,sex:np.array,sens:float,spec:float,theta_start:np.array,X_start:np.array,kick_scaling:float,epsilon:float,L:int,mu:np.array,prior_X_0:float,K:int,K_latent:int,K_chunk:int,zarr_names_start:str,seed:int) :
    ...
    # kick_scaling - how much to scale the momentum proposal covariance matrix by
    # epsilon - jump size in the HMC dynamics
    # L - number of jumps in the HMC dynamics
    ...
    for k_chunk in range(K_chunk):
        ...
        for k in range(k_start,k_end):
            ...
            # propose a starting momentum (kick)
            p = random.multivariate_normal(np.zeros(4),kick_scaling*np.identity(4))
            
            # initial conditions for dynamics
            p_moving = p
            theta_moving = theta

            # run the dynamics for L times
            for t in range(L):
                p_half = p_moving - epsilon*0.5*grad_potential_jit(X,seasonal_matrix_G,seasonal_matrix_H,age,sex,theta_moving,T,N,h,mu)
                theta_moving = theta_moving + epsilon*p_half
                p_moving = p_half - epsilon*0.5*grad_potential_jit(X,seasonal_matrix_G,seasonal_matrix_H,age,sex,theta_moving,T,N,h,mu)

            # the proposed position and momentum after the dynamics
            p_prop = p_moving
            theta_prop = theta_moving
            ...
            # log proposal ratio
            l_proposal_ratio = np.sum(p**2)/2 - np.sum(p_prop**2)/2

            # M-H step to potentially update theta
            log_alpha = ll_prop - ll_curr + l_proposal_ratio
            log_u = np.log(random.uniform())
            if log_u < log_alpha :
                theta = theta_prop
                ll_curr = ll_prop
