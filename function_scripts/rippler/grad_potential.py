...
def grad_potential_jax(X:np.array,seasonal_matrix_G:np.array,seasonal_matrix_H:np.array,age:np.array,sex:np.array,theta:np.array,T:int,N:int,h:np.array,mu:np.array) :
    ...
    grad_potential = jnp.sum(not_infect_stack,axis=(1,2)) - jnp.sum(infect_stack,axis=(1,2))

    # adding the prior part to the output
    grad_potential = grad_potential + jnp.array([mu[0],mu[1],mu[2]*(theta[2]>0)-mu[2]*(theta[2]<0),mu[3]*(theta[3]>0)-mu[3]*(theta[3]<0)])

    # adding -inf to the output if either beta is less than 0
    grad_potential = grad_potential + jnp.array([jnp.nan_to_num((theta[0]<=0)*(-1)*jnp.inf,nan=0,neginf=jnp.inf*(-1)),jnp.nan_to_num((theta[1]<=0)*(-1)*jnp.inf,nan=0,neginf=jnp.inf*(-1)),0,0])
...
grad_potential_jit = jit(grad_potential_jax, static_argnames=["T","N"])