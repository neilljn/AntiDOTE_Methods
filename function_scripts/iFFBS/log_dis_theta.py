...
def log_dis_theta_jax(X:np.array,seasonal_matrix_G:np.array,seasonal_matrix_H:np.array,age:np.array,sex:np.array,theta:np.array,T:int,N:int,h:np.array,mu:np.array) :
...
log_dis_theta_jit = jit(log_dis_theta_jax, static_argnames=["T","N"])